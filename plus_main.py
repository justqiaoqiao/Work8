...
def prepare_posedirs(posedirs: torch.Tensor, expected_pose_dim: int):
    """
    不同版本/实现里 posedirs 的形状有时写作 [P, V*3]，有时会以转置形式出现。
    官方 lbs() 需要的是 [P, V*3]。
    """
    if posedirs.dim() != 2:
...
def generate_animation(model, faces, betas, out_dir, joint_idx=18, num_frames=30):
    anim_dir = os.path.join(out_dir, "animation")
    make_out_dir(anim_dir)
    frames = []
    
    # 使用正弦平滑（慢入慢出），角度从 0 到 -1.5 弧度（向内弯曲）
    t = np.linspace(0, 1, num_frames)
    angles = -(np.sin(t * np.pi - np.pi/2) + 1) / 2 * 1.5 
    
    print(f"正在生成动画帧 (已优化旋转轴与方向)，保存至 {anim_dir}...")
    for i, angle in enumerate(angles):
        body_pose = torch.zeros((1, 69))
        
        # 优化：SMPL 手肘弯曲通常在第 0 个分量 (X轴)
        # 将 + angle 改为 - angle 或调整轴索引即可修复反方向
        body_pose[0, (joint_idx-1)*3 + 0] = angle 
        
        data = compute_manual_lbs(model, betas, torch.zeros((1,3)), body_pose)
        
        frame_path = os.path.join(anim_dir, f"frame_{i:03d}.png")
        save_single_figure(
            frame_path,
            to_numpy(data["verts"][0]),
            faces,
            title=f"Joint {joint_idx} Flexion: {int(np.degrees(abs(angle)))}°"
        )
        frames.append(imageio.imread(frame_path))
    
    imageio.mimsave(os.path.join(out_dir, "lbs_animation_optimized.gif"), frames, duration=0.1)
    print("优化版动画生成完成：outputs/lbs_animation_optimized.gif")
...
def main(args):
    ...
    # 选做：生成动画
    generate_animation(model, faces, betas, out_dir, joint_idx=18, num_frames=30)
